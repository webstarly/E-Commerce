...
const UploadProducts = ({ onClose , fetchData }) => {
  ...
  const handleSubmit = async(e) =>{
    ...
    const response = await fetch(FullApi.productUpload.url,{
      method : FullApi.productUpload.method,
      credentials : "include",
      headers : {
  ...
  return (
    <div className="fixed h-full w-full bg-slate-200 bg-opacity-30 top-0 left-0 bottom-0 right-0 justify-center items-center flex">
      <div className="bg-white p-4 h-full max-h-[80%] w-full max-w-2xl overflow-hidden">
        ...
        <form className="grid p-4 gap-3 overflow-y-scroll h-full" onSubmit={handleSubmit}>
